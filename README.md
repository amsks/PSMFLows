...
and our implementations of four baselines (IQL, ReBRAC, IFQL, and SAC)
can also be found in the same directory.

### Configuration (Hydra)

Runs are configured with [Hydra](https://hydra.cc). The config tree is:

```
configs/
  config.yaml            # run parameters (seed, env_name, *_steps, *_interval, ...)
  agent/                 # one file per agent (the selectable config group)
    fql.yaml  iql.yaml  ifql.yaml  rebrac.yaml  sac.yaml
```

Select the agent with `agent=<name>` and override any field with dotted keys:

```bash
# default agent is fql
python main.py env_name=cube-double-play-singletask-v0 agent.alpha=300 seed=0
# switch agent via the config group
python main.py agent=iql env_name=cube-double-play-singletask-v0 agent.alpha=10
```
...
* For other hyperparameters, you may use the default values in `configs/agent/fql.yaml`.
